...
public class SingleNeuronPerceptron {
    public static void main(String[] args) {
        ...
        System.out.printf("starting perceptron weights: %s%n", Arrays.toString(neuron.weightsAndT));
        ...
        if(testSet == null) {
            ...
            System.out.printf("please enter %d values seperated by white characters:%n", neuron.size);
            ...
            try {
                while (true){
                    ...
                    if (inputValues.length != neuron.size) {
                        System.out.printf("Expected %d values, got %d values%n", neuron.size, inputValues.length);
                    }
                    Double[] parsedData = new Double[inputValues.length];
                    for (int i = 0; i < parsedData.length; i++) {
                        parsedData[i] = Double.parseDouble(inputValues[i]);
                    }
                    int guess = mostAccurate.classify(parsedData);
            ...
            System.out.println(mostAccurate.classify(test,true));
